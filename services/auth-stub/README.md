...
Write a contract parity check alongside the real service in Sprint 8. It signs with both implementations and cross-verifies: a stub token must verify in the service, a service token must verify in the stub, and the claim names, types, algorithm and lifetime must match. A test of that shape fails the moment either side drifts, which is the difference between finding the drift in a test run and finding it during the cutover.
