...
#
# The harness builds a scratch database inside the running Postgres container,
# applies every migration and every seed file to it, and asserts behaviour
# against the names you declared in manifest.env. Your working database is not
# touched. The scratch database is dropped when everything passes, and left in
# place when something fails so that you can look at it.
#
# Passing these checks is necessary and not sufficient. Third normal form, the
# quality of an index justification and your ability to walk the model are
# assessed in the design review.
...
[ -f "${MANIFEST}" ] || abort \
    ...
    "The harness reads your table and column names from that file. If you have" \
    "deleted it, restore it from the repository."
...
MANIFEST_KEYS="SCHEMA_NAME ACCOUNTS_TABLE ACCOUNTS_STATUS_COLUMN
...
if [ -n "${OUTSTANDING}" ]; then
    abort "manifest.env is not filled in." \
        ...
        "catalogue. Postgres folds unquoted identifiers to lower case." \
...
if [ "${#MIGRATIONS[@]}" -eq 0 ]; then
    abort "No migration files in migrations/." \
        ...
        "in filename order. Create migrations/001_create_core_tables.sql and put" \
        "your DDL in it. See migrations/README.md."
fi
...
if [ "${#SEEDS[@]}" -eq 0 ]; then
    abort "No seed files in seed/." \
        "Seed data is part of the deliverable and half the later checks need it." \
        "See seed/README.md for what it has to cover."
fi
pass "seed/ holds ${#SEEDS[@]} SQL file(s)"
...
section "Rebuilding from migrations into ${CHECK_DB}"
...
if "${SCRIPT_DIR}/apply.sh" --fresh --database "${CHECK_DB}" >"${APPLY_LOG}" 2>&1; then
    pass "every migration and seed file applies cleanly to an empty database"
else
    printf '\n'
    sed 's/^/  | /' "${APPLY_LOG}"
    abort "The migrations or the seed data did not apply to an empty database." \
        "The output above is from applying them, in filename order, to a database" \
        "with nothing in it. That is the state a teammate cloning the repository" \
        "starts from, so this failure is a real one rather than an artefact of" \
        "the harness." \
...
for state in "${ACCOUNTS_STATUS_ACTIVE}" "${ACCOUNTS_STATUS_SUSPENDED}" "${ACCOUNTS_STATUS_CLOSED}"; do
    ...
    if [ "${count:-0}" -ge 1 ]; then
        pass "${count} seeded account(s) in state ${state}"
    else
...
if [ -n "${MISSING_STATES}" ]; then
    ...
    fail "No seeded account in state(s):${MISSING_STATES}" \
        "States present in the seed data: ${present:-none}" \
        "The refusal paths for a frozen account and a finished account cannot be" \
        "tested in Sprint 6 against data that has neither." \
...
if [ "${ORDER_COUNT:-0}" -ge 1 ]; then
    pass "${ORDER_COUNT} seeded order(s)"
else
    fail "No seeded orders." \
        "Seed orders in more than one lifecycle state, including one that was" \
        "rejected. Sprint 4 analyses this data and Sprint 6 tests against it."
fi
