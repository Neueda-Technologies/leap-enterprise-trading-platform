...
against your daily quota. A test that fails only when the API is slow is a
test nobody trusts.
...
| File | Provenance |
|---|---|
| `candles-aapl-2026-07.json` | Nine US trading days in July 2026, shaped from the published `CandlesResponse` schema, prices invented. The 3rd of July is absent because the US market was shut, so the series has a calendar gap. |
| `candles-infy-ns-2026-07.json` | Eight NSE trading days over the same period in a second currency, shaped from the same schema, prices invented. One candle reports a null volume and one is flagged `synthetic`, both of which the live API does emit. |
| `candles-malformed.json` | The same envelope with the payload deliberately corrupted, written for the malformed-input test the acceptance criteria require. Prices invented. |
...
it, or raising an error are all defensible, and they are not equally
defensible for all six defects. What is not defensible is loading a row that
says a share traded at a high below its low, and then drawing a chart from it.
Whichever you choose, the behaviour has to be asserted in a test.
