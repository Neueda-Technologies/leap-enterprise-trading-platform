...
#
# The harness builds a scratch virtual environment, installs this package into
# it, and runs your test suite there. It touches nothing outside this folder,
# needs no database and no container, and never calls the Fauxnance API, so it
# costs nothing against your daily quota.
#
...
PACKAGE_NAME="${PACKAGE_NAME:-analytics}"
...
if [ -n "${OUTSTANDING}" ]; then
    abort "manifest.env is not filled in." \
        ...
        "Declare the three pipeline functions as module:function, and the" \
        "malformed-input test as a pytest node id. Examples are in the file." \
...
[ -f "${SPRINT_DIR}/pyproject.toml" ] || abort \
    "No pyproject.toml in ${SPRINT_DIR}." \
    "The package has to declare its dependencies and its build so that a" \
    "teammate can install it. Restore the file from the repository."
pass "pyproject.toml is present"

PACKAGE_DIR="${SRC_DIR}/${PACKAGE_NAME}"
if [ -d "${PACKAGE_DIR}" ]; then
    pass "src/${PACKAGE_NAME}/ exists"
else
    abort "No package directory at src/${PACKAGE_NAME}." \
        "PACKAGE_NAME in manifest.env says ${PACKAGE_NAME}, and nothing by that" \
        "name is under src/. Either correct the manifest or move the package."
fi
...
if [ "${TEST_FILES}" -ge 1 ]; then
    ...
    abort "No test files in tests/." \
        ...
        "step, including one malformed-input case. Name the files test_*.py."
fi

MISSING_FIXTURES=""
for fixture in candles-aapl-2026-07.json candles-infy-ns-2026-07.json candles-malformed.json; do
...
python3 -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 12) else 1)' || abort \
    ...
    "pyproject.toml requires 3.12 or later. Install a current Python, or point" \
    "the harness at one by putting it first on your PATH."
...
SCAN_TARGETS=""
for target in "${SRC_DIR}" "${TESTS_DIR}" "${FIXTURES_DIR}" "${CLAIMS}" "${MANIFEST}" "${SPRINT_DIR}/pyproject.toml"; do
...
scan_for() {
    ...
    grep -rInE "$1" ${SCAN_TARGETS} 2>/dev/null | sed "s|^${SPRINT_DIR}/||" || true
}
...
if [ "${#LEAKS[@]}" -eq 0 ]; then
    pass "no key literal, and no base URL carrying a key, in src, tests, fixtures or claims.md"
else
...
# The name has to appear in code rather than in the prose the scaffold ships
# with, so this parses each module and ignores docstrings and comments.
ENV_READS="$(python3 - "${SRC_DIR}" <<'PY' 2>/dev/null || true
import ast
import pathlib
import sys
...
if [ -n "${ENV_READS}" ]; then
    ...
    fail "No code under src/ names FAUXNANCE_API_KEY." \
        ...
        "Docstrings and comments do not count, which is why the scaffold's own" \
        "prose does not satisfy this check. The check is still a weak one: it" \
        "proves the name is used, not that it is the only source of the key."
fi
...
if [ "${REUSE_VENV}" -eq 1 ] && [ -x "${VENV_PY}" ]; then
    ...
    if "${VENV_PY}" -m pip install --quiet --disable-pip-version-check \
        ...
        abort "pip could not install the package." \
            "The output above is from installing it into an environment with" \
            "nothing in it, which is the state a teammate cloning the" \
            "repository starts from. A package that installs only on the" \
            "machine it was written on is not a deliverable." \
    ...
    case "${label}:${status}" in
        ...
        *:IMPORT_FAILED)
            fail "The module holding ${label} would not import: ${detail}" \
                ...
                "is usually a path assumption or a missing dependency in" \
                "pyproject.toml." ;;
        *:NO_ATTRIBUTE)
...
if [ "${TEST_COUNT}" -ge "${MIN_TESTS}" ]; then
    ...
    fail "Only ${TEST_COUNT} test(s) collected, and ${MIN_TESTS} is the minimum." \
        ...
        "The example test shipped in tests/ counts towards the number and" \
        "proves nothing about your code. Replace it."
fi
