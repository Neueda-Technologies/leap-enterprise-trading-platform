...
#
# The harness builds this module from a clean state, runs your suite, reads
# the compiled classes to check the enumerations and the exception hierarchy,
# and reads the dependency tree to check that nothing forbidden reached the
...
#
# Passing these checks is necessary and not sufficient. Whether the eight
# rules are correct, whether the tests came before the code, and whether the
# diagrams describe the code you wrote are assessed at the design review.
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        -h|--help) sed -n '2,21p' "${BASH_SOURCE[0]}" | sed 's/^#\{1,2\} \{0,1\}//'; exit 0 ;;
...
if [ -n "${OUTSTANDING}" ]; then
    abort "manifest.env is not filled in." \
        ...
        "exception classes. Defaults are in the file."
fi
...
[ -f "${SPRINT_DIR}/pom.xml" ] || abort \
    ...
    "The module has to declare its build so that a teammate can compile it." \
...
if [ "${MAIN_SOURCES}" -ge 1 ]; then
    ...
    abort "No Java source under src/main/java." \
        "The scaffold ships empty on purpose: the four entities, the three" \
        "enumerations, the DTO, the exception hierarchy and the rules are" \
        "yours to design from the brief." \
...
if (cd "${SPRINT_DIR}" && mvn "${MVN_FLAGS[@]}" clean test-compile) >"${BUILD_LOG}" 2>&1; then
    ...
    abort "The build does not succeed from a clean state." \
        ...
        "If the failure is the banned-dependencies rule, read the comment" \
        "beside that rule in pom.xml before changing anything."
fi
...
if [ -n "${MISSING_CLASSES}" ]; then
    fail "No test report for:${MISSING_CLASSES}" \
        ...
        "name and that it holds at least one test method."
fi
...
# every JDBC driver a team is likely to reach for. The enforcer rule in
# pom.xml already fails the build on these; this reads the resolved tree so
# that a rule somebody weakened still shows up here.
...
if (cd "${SPRINT_DIR}" && mvn "${MVN_FLAGS[@]}" dependency:tree -DoutputType=text) >"${TREE_LOG}" 2>&1; then
    ...
    if [ -z "${HITS}" ]; then
        pass "no Spring, servlet or JDBC artefact in the resolved dependency tree"
    else
