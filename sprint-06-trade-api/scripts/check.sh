...
if [ -n "${OUTSTANDING}" ]; then
    abort "manifest.env is not filled in." \
        ...
        "Every key has a default in the file. Set the ones your team decided" \
...
[ -f "${SPRINT_DIR}/pom.xml" ] || abort \
    ...
    "The service has to declare its build so that a teammate can compile it." \
...
if [ "${MAIN_SOURCES}" -ge 1 ]; then
    pass "src/main/java holds ${MAIN_SOURCES} source file(s) beyond the package descriptions"
else
    abort "No Java source under src/main/java." \
        "The scaffold ships as empty packages on purpose: the controllers, the" \
        "services, the mappers, the DTOs and the token verification are yours" \
        "to design from the contract and the brief." \
...
if [ "${TEST_SOURCES}" -ge 1 ]; then
    ...
    fail "No Java source under src/test/java." \
        ...
        "error envelope, and a mapper test against the in-memory database the" \
        "build already provides. A deliverable with no tests is assessed as" \
        "one."
fi
...
if [ -f "${DOCKERFILE_PATH}" ]; then
    ...
    fail "No ${DOCKERFILE} in ${SPRINT_DIR}." \
        ...
        "Dockerfile. Dockerfile.example is the naive single-stage version and" \
        "is not it: read the comments in it and write your own." \
...
if [ -z "${DOMAIN_JAR}" ] || [ ! -f "${DOMAIN_JAR}" ]; then
    fail "${DOMAIN_GROUP_ID}:${DOMAIN_ARTIFACT_ID}:${DOMAIN_VERSION} is not on this service's compile classpath." \
        ...
        "places: sprint-05-domain-engine/pom.xml, the domain.* properties in" \
...
if [ ! -f "${DOCKERFILE_PATH}" ]; then
    ...
    if [ "${FROM_COUNT}" -lt 2 ]; then
        fail "${DOCKERFILE} has ${FROM_COUNT} stage(s), and a multi-stage build has more than one." \
            ...
            "Read the comments in Dockerfile.example. They list what is wrong" \
            "with the naive version, which is this one."
    else
