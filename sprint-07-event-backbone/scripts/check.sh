...
# reads the commit history for the characterisation tests, and scans every file
# in this folder for a Fauxnance key.
#
...
# whether the quota arithmetic holds and whether the refactoring improved
# anything are assessed at the review.
...
# A characterisation suite that pins one loader is more than two tests.
...
STATIC_KEYS="EXECUTOR_DIR EXECUTOR_BASE_PACKAGE EXECUTOR_CONSUMER_GROUP
POLLER_DIR POLLER_PACKAGE_NAME POLL_INTERVAL_SECONDS STARTER_DIR
STARTER_TEST_DIR ORDERS_TOPIC ORDERS_PARTITIONS TRADE_EVENTS_TOPIC
TRADE_EVENTS_PARTITIONS MARKET_DATA_TOPIC MARKET_DATA_PARTITIONS"
...
if [ -n "${OUTSTANDING}" ]; then
    abort "manifest.env is not filled in." \
        ...
        "Everything except the three ETL commands ships with a working" \
        "default. The commands have no defensible default, because the" \
...
STARTER_PATH="${SPRINT_DIR}/${STARTER_DIR}"
STARTER_TEST_PATH="${SPRINT_DIR}/${STARTER_TEST_DIR}"
...
[ -f "${EXECUTOR_PATH}/pom.xml" ] || abort \
    ...
    "The executor has to declare its build so that a teammate can compile it." \
    "Restore the file from the repository, or correct EXECUTOR_DIR in" \
    "manifest.env if you moved the service."

[ -f "${POLLER_PATH}/pyproject.toml" ] || abort \
    "No pyproject.toml at ${POLLER_DIR}/." \
    "Restore it from the repository, or correct POLLER_DIR in manifest.env."

[ -d "${STARTER_PATH}" ] || abort \
    "Nothing at ${STARTER_DIR}." \
    "That is the code this sprint asks you to characterise and refactor. If" \
    "you moved it, correct STARTER_DIR in manifest.env. If you deleted it," \
    "restore it: deleting the subject does not complete the exercise."

pass "the executor build, the poller project and the starter are all present"
...
if [ "${EXECUTOR_SOURCES}" -ge 1 ]; then
    pass "${EXECUTOR_DIR} holds ${EXECUTOR_SOURCES} source file(s) beyond the package descriptions"
else
    abort "No Java source under ${EXECUTOR_DIR}/src/main/java." \
        "The scaffold ships as empty packages on purpose. The listener, the" \
        "fill rule, the settlement, the quote client and the producer are" \
        "yours to design from the brief and from contracts/kafka-topics.md." \
        ...
        "come back." \
        "Each package carries a package-info.java saying what belongs in it." \
        "Those do not count towards this number."
fi

POLLER_PACKAGE_PATH="${POLLER_PATH}/src/${POLLER_PACKAGE_NAME}"
[ -d "${POLLER_PACKAGE_PATH}" ] || abort \
    "No package directory at ${POLLER_DIR}/src/${POLLER_PACKAGE_NAME}." \
...
if [ "${POLLER_STATEMENTS:-0}" -ge 1 ]; then
    pass "the poller package holds code, not only the scaffold's docstrings"
else
    abort "Every module under ${POLLER_DIR}/src/${POLLER_PACKAGE_NAME} is still a docstring and nothing else." \
        "That is what the scaffold ships: each module says what belongs in it" \
        "and none of them does it yet." \
        "On day one of the sprint this is the expected result. The poller is" \
        "the smallest of the four deliverables and the one to write first, on" \
        "the grounds that a topic with nothing on it is hard to debug against."
fi
...
if [ "${REUSE_VENV}" -eq 1 ] && [ -x "${VENV_PY}" ]; then
    ...
    if "${VENV_PY}" -m pip install --quiet --disable-pip-version-check \
        -e "${POLLER_PATH}[dev]" -e "${SPRINT_DIR}/etl-starter[dev]" \
        >"${WORK_LOG}" 2>&1; then
...
section 'Characterisation tests for the starter'

CHARACTERISATION_FILES=()
while IFS= read -r file; do
    [ -n "${file}" ] || continue
    CHARACTERISATION_FILES[${#CHARACTERISATION_FILES[@]}]="${file}"
done < <(find "${STARTER_TEST_PATH}" -type f -name 'test_*.py' 2>/dev/null | LC_ALL=C sort)

CHARACTERISATION_COUNT=0

if [ "${#CHARACTERISATION_FILES[@]}" -eq 0 ]; then
    fail "No test files under ${STARTER_TEST_DIR}." \
        "The deliverable is characterisation tests first and refactoring" \
        "second. A characterisation test does not assert what the loader" \
        "should do. It records what it does now, including the parts you" \
        "disagree with, so that a refactoring that changes behaviour by" \
        "accident fails loudly instead of quietly." \
        "Name them test_*.py and put them in ${STARTER_TEST_DIR}, or point" \
        "STARTER_TEST_DIR somewhere else."
else
    COLLECTED="$( (cd "${SPRINT_DIR}/etl-starter" && "${VENV_PY}" -m pytest --collect-only -q) 2>&1 || true)"
    CHARACTERISATION_COUNT="$(printf '%s\n' "${COLLECTED}" | grep -c '::' || true)"

    if [ "${CHARACTERISATION_COUNT}" -ge "${MIN_CHARACTERISATION_TESTS}" ]; then
        pass "${CHARACTERISATION_COUNT} characterisation test(s) collected"
    else
        fail "Only ${CHARACTERISATION_COUNT} test(s) collected under ${STARTER_TEST_DIR}, and ${MIN_CHARACTERISATION_TESTS} is the minimum." \
            "Pinning one batch loader takes more than that. The load it" \
            "produces from a known set of source rows, what it does with a row" \
            "it cannot place, and what a second run does to a warehouse that" \
            "already holds the first are three separate observations."
    fi

    if (cd "${SPRINT_DIR}/etl-starter" && "${VENV_PY}" -m pytest -q) >"${WORK_LOG}" 2>&1; then
        pass "the characterisation suite passes"
    else
        printf '\n'
        tail -n 25 "${WORK_LOG}" | sed 's/^/  | /'
        fail "The characterisation suite does not pass." \
            "A characterisation test that fails is telling you one of two" \
            "things: either it records behaviour the loader never had, or the" \
            "refactoring changed behaviour. Both are worth knowing and neither" \
            "is finished." \
            "Reproduce it with:" \
            "  cd ${SPRINT_DIR}/etl-starter && ${VENV_PY} -m pytest"
    fi
fi

# --- the order the work happened in ------------------------------------------

section 'Tests before refactoring'
...
if [ -z "${REPO_ROOT}" ]; then
    ...
    note "commit that changes the starter. Without a history there is nothing"
    ...
    STARTER_COMMITS="$(git -C "${REPO_ROOT}" log --reverse --format=%H -- "${STARTER_PATH}" 2>/dev/null || true)"
    STARTER_INTRODUCED="$(printf '%s\n' "${STARTER_COMMITS}" | sed -n '1p')"
    STARTER_FIRST_CHANGE="$(printf '%s\n' "${STARTER_COMMITS}" | sed -n '2p')"
    TESTS_ADDED="$(first_commit_for "${CHARACTERISATION_FILES[@]}")"

    if [ -z "${STARTER_INTRODUCED}" ]; then
        skip "the ordering check: no commit in this history touches ${STARTER_DIR}."
        ...
        fail "The characterisation tests exist on disk and in no commit." \
            ...
            "committed cannot satisfy it. Commit them before you touch the" \
            "loader."
    elif [ -z "${STARTER_FIRST_CHANGE}" ]; then
        pass "the characterisation tests are committed and the starter is unchanged since it arrived"
        note "the ordering rule holds so far because nothing has refactored the"
        note "loader yet. The tests are the first half of the deliverable and"
        note "the refactoring is the second."
    elif [ "${TESTS_ADDED}" = "${STARTER_FIRST_CHANGE}" ]; then
        fail "The characterisation tests and the first change to the starter are the same commit." \
            "Commit: $(git -C "${REPO_ROOT}" log -1 --format='%h %s' "${TESTS_ADDED}")" \
            "Before means before. Tests written alongside a change record what" \
            "the code does after the change, which is the one thing they" \
            "cannot be used to check." \
            "Split the commit: the tests against the loader as it stands, then" \
            "the change."
    elif git -C "${REPO_ROOT}" merge-base --is-ancestor "${TESTS_ADDED}" "${STARTER_FIRST_CHANGE}" 2>/dev/null; then
        pass "the characterisation tests were committed before the first change to ${STARTER_DIR}"
        note "tests   $(git -C "${REPO_ROOT}" log -1 --format='%h %ad %s' --date=short "${TESTS_ADDED}")"
        note "change  $(git -C "${REPO_ROOT}" log -1 --format='%h %ad %s' --date=short "${STARTER_FIRST_CHANGE}")"
    else
        fail "The starter was changed before the characterisation tests were committed." \
            "change  $(git -C "${REPO_ROOT}" log -1 --format='%h %ad %s' --date=short "${STARTER_FIRST_CHANGE}")" \
            "tests   $(git -C "${REPO_ROOT}" log -1 --format='%h %ad %s' --date=short "${TESTS_ADDED}")" \
            "The rule, exactly: the first commit that adds a test file under" \
            "${STARTER_TEST_DIR} has to be an ancestor of the first commit" \
            "after the scaffold that touches ${STARTER_DIR}." \
            "It is not bureaucracy. A test written after a refactoring records" \
            "the behaviour of the refactored code, so it cannot tell you" \
            "whether the refactoring changed anything. That is the whole" \
            "purpose of the technique, and the history is the only evidence" \
            "that it was used."
    fi
fi
...
if [ "${FAILED}" -eq 0 ]; then
    ...
    printf 'SonarQube gate, and whether the refactoring left the loader better\n'
    printf 'than it found it.\n'
    exit 0
fi

printf '\nEach failure above says what was expected and where to look. The\n'
printf 'SonarQube gate, the fill rule and the quality of the refactoring are\n'
printf 'assessed at the review, not here.\n'
