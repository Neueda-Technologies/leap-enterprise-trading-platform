...
# decodes the access token claim by claim, refreshes and replays, times a wrong
# password against an unknown user, reads a stored hash back, asks your Trade
# REST API to accept a token, and fetches the OpenAPI document the service
# serves.
#
...
UNIFORM_TIMING_FLOOR_MS THROTTLE_COOLDOWN_SECONDS
...
if [ -n "${OUTSTANDING}" ]; then
    abort "manifest.env is not filled in." \
        ...
        "documented as optional are STORED_HASH_SQL, CUTOVER_BASELINE_REF and" \
        "STUB_DEV_SECRET, and an empty value there turns a probe into a skip."
fi
...
for key in UNIFORM_TIMING_ATTEMPTS UNIFORM_TIMING_THRESHOLD_MS UNIFORM_TIMING_RATIO \
    UNIFORM_TIMING_FLOOR_MS SECURITY_REVIEW_NONE_MIN_WORDS; do
    eval "value=\${${key}}"
    if [ -n "${value}" ]; then
...
[ -f "${SPRINT_DIR}/package.json" ] || abort \
    ...
    "The service has to declare its build and its dependencies so that a" \
    "teammate can install and run it. Restore the file from the repository."
...
count_spec() {
    find "$1" "${SPRINT_DIR}/test" -type f -name '*.spec.ts' 2>/dev/null | wc -l | tr -d ' '
}
...
if [ "${SOURCE_COUNT}" -ge 1 ]; then
    ...
    abort "No TypeScript source under ${SRC_DIR}." \
        "The scaffold ships as empty module directories on purpose: the" \
        "controller, the guard, the token service, the hashing, the repositories" \
        "and the DTOs are yours to design from the contract and the brief." \
        "On day one of the sprint this is the expected result. Write the entry" \
        "point and one route, then come back." \
        "Each directory carries a README saying what belongs in it. Those are" \
        "not counted here."
fi
...
if [ "${SPEC_COUNT}" -ge 1 ]; then
    ...
    fail "No *.spec.ts anywhere under ${SRC_DIR} or test/." \
...
if [ -f "${TEMPLATE_PATH}" ]; then
    ...
    fail "No ${SECURITY_REVIEW_TEMPLATE} in ${SPRINT_DIR}." \
        ...
        "the scaffold. Restore it from the repository, or correct" \
...
if [ "${NODE_MAJOR}" -ge 20 ]; then
    ...
    abort "Node ${NODE_MAJOR} is older than the 20 this service declares." \
        "package.json sets engines.node to >=20, and NestJS 11 needs it."
fi
...
if (cd "${SPRINT_DIR}" && npm run build) >"${BUILD_LOG}" 2>&1; then
    ...
    fail "The build does not succeed." \
        ...
        "The build compiles src/ through tsconfig.build.json, which excludes" \
        "your specs. A service that compiles only when the tests are included" \
        "has a source file importing something from a spec."
fi
        ...
        if [ "${HTTP_STATUS}" != "200" ]; then
            ...
            if [ -n "${SECOND_REFRESH}" ] && [ "${SECOND_REFRESH}" != "${REFRESH_TOKEN}" ]; then
                ...
                fail "The refresh returned the same refresh token." \
                    "Criterion 5 is that refresh tokens rotate. Reissuing only" \
                    "the access token leaves a seven-day credential in place," \
                    "and a stolen one is usable for its whole life without the" \
                    "real user noticing anything."
            fi

            # The new token is exercised before the old one is replayed. A
            # replay is meant to revoke the whole chain, which would take the
            # new token with it, so the order proves both without one probe
            # destroying the other.
            request POST "${SERVICE_URL}/auth/refresh" "" \
                "$(printf '{"refreshToken":"%s"}' "${SECOND_REFRESH}")"
            THIRD_REFRESH=""
            if [ "${HTTP_STATUS}" = "200" ]; then
                pass "the refresh token the rotation issued works"
                THIRD_REFRESH="$(json_string "${HTTP_BODY}" refreshToken)"
            else
                fail "The refresh token the rotation issued was refused: HTTP ${HTTP_STATUS}." \
                    "Body: ${HTTP_BODY:-empty}" \
                    "Rotation that invalidates the old token and does not honour" \
                    "the new one logs every user out every fifteen minutes."
            fi
            ...
            if [ "${HTTP_STATUS}" = "200" ]; then
                fail "A refresh token that has already been exchanged still works." \
                    "Criterion 5. Rotation is not only issuing a new token, it" \
                    "is killing the one that was presented. Two refresh tokens" \
                    "that both work are two live sessions, and if one of them" \
                    "was stolen the theft is now permanent: the thief refreshes" \
                    "for seven days and the real user notices nothing, because" \
                    "their own token still works." \
                    "Consume the presented token in the statement that reads it," \
                    "and treat a second presentation as theft rather than as a" \
                    "retry."
            else
            ...
            if [ "${REPLAY_REFUSED}" -eq 1 ] && [ -n "${THIRD_REFRESH}" ]; then
                ...
                if [ "${HTTP_STATUS}" = "200" ]; then
                    ...
                    note "contract treats a second presentation as theft, because"
                    note "nothing can tell it from a repeated request, and"
                    note "revoking the chain is what ends the stolen session."
                else
...
if [ "${FAILED}" -eq 0 ]; then
    if [ "${LIVE}" -eq 0 ]; then
        ...
        printf 'rotated a refresh token and timed two failures, without knowing\n'
    ...
    printf 'whether the replay revokes the chain rather than one token, whether the\n'
    printf 'security review is a reading of your service, and whether the cutover\n'
    printf 'changed configuration and nothing else.\n'
    exit 0
fi
