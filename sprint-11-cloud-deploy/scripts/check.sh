    ...
    stage_check() {
        ...
        if grep -qEi "${stage_pattern}" ${DEPLOY_FILES} 2>/dev/null; then
            ...
            fail "No ${stage_label} stage in the deployment." \
                ...
                "is what an asynchronous assessment reads. A workflow that calls" \
                "a script keeps its stages in two files: name the script in" \
                "DEPLOY_SUPPORTING_FILES and both are searched."
        fi
    }
